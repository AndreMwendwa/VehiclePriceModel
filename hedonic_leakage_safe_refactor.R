...
### Explained Variance ############
###################################
pca_var_new <- pca_new$sdev^2
pca_var_explained_new <- (pca_var_new / sum(pca_var_new)) * 100
pca_var_table_new <- tibble(
  Dimension = paste0("PC", seq_along(pca_var_explained_new)),
  Variance_Explained_Percent = round(pca_var_explained_new, 2)
)
print(pca_var_table_new)

# USED cars PCA
pca_var_used <- pca_used$sdev^2
pca_var_explained_used <- (pca_var_used / sum(pca_var_used)) * 100
pca_var_table_used <- tibble(
  Dimension = paste0("PC", seq_along(pca_var_explained_used)),
  Variance_Explained_Percent = round(pca_var_explained_used, 2)
)
print(pca_var_table_used)
